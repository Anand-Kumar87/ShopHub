...
import {
    FiPlus, FiEdit2, FiTrash2, FiImage, FiX, FiCheck, FiUploadCloud, FiLayers, FiTag, FiSearch
} from 'react-icons/fi';

export default function CategoryManagement() {
    const cachedCats = adminCache.get('admin_categories');
    const cachedProds = adminCache.get('admin_products');
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            if (!cachedCats) setIsLoading(true);
            try {
                ...
                if (realProducts) {
                    ...
                    adminCache.set('admin_products', realProducts);
                }
    ...
    const handleCategorySubmit = async (e) => {
        ...
        if (selectedCategory) {
            ...
            const updated = categories.map(c => c.id === selectedCategory.id ? { ...c, ...finalCategoryForm } : c);
            setCategories(updated);
            adminCache.set('admin_categories', updated);
            ...
            const updated = [newCatObj, ...categories];
            setCategories(updated);
            adminCache.set('admin_categories', updated);
        ...
        adminCache.set('admin_products', updatedProducts);
    ...
    const deleteCategory = async (id, slug) => {
        if (window.confirm("Delete this collection? Linked products will become uncategorized.")) {
            ...
            adminCache.set('admin_categories', updatedCats);
            adminCache.set('admin_products', updatedProds);
    ...
    return (
        <>
            <div className="animate-fade-in space-y-8">
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-stone-200 pb-4">
                    <div>
                        ...
                        <p className="text-xs text-stone-400 mt-0.5">{categories.length} Total Collections</p>
                    </div>
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            ...
                            className="bg-stone-900 text-white text-[10px] font-bold tracking-widest uppercase px-5 py-2.5 rounded-full flex items-center gap-2 hover:bg-stone-800 transition-colors shadow-lg shadow-stone-900/10 flex-shrink-0"
                        >
                            <FiPlus size={14} /> Add
                        </button>
                    </div>
                                ...
                                <tr>
                                    <td colSpan="4" className="text-center py-16">
                                        ...
                                        <p className="text-stone-400 text-sm font-medium">
                                            {searchQuery ? 'No collections match your search filter.' : 'No collections created yet.'}
                                        </p>
                                    </td>
                                </tr>
                            )}
            ...
            {/* 🚀 PREMIUM MODAL WITH STICKY BUTTON (PORTALED) */}
            {isMounted && isCategoryModalOpen && createPortal(
                <div className="fixed inset-0 bg-stone-900/70 backdrop-blur-md z-[999999] flex items-center justify-center p-3 sm:p-6 overflow-y-auto animate-fade-in">
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden border border-stone-100 flex flex-col max-h-[90vh] my-auto">
