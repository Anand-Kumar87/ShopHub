...
import {
    FiPlus, FiEdit2, FiTrash2, FiImage, FiX, FiCheck, FiUploadCloud, FiSearch
} from 'react-icons/fi';
...
export default function ProductManagement() {
    ...
    // 0ms Cache Load + Stale-While-Revalidate Live Fetch
    useEffect(() => {
        const cachedProducts = adminCache.get('admin_products');
        const cachedCategories = adminCache.get('admin_categories');
        if (cachedProducts) {
        ...
        const fetchInitialData = async () => {
            if (!cachedProducts) setIsLoading(true);
            try {
                ...
                if (realProducts && !prodError) {
                    ...
                    const mapped = sortedProducts.map(p => ({ ...p, images: p.images || (p.image_url ? [p.image_url] : []) }));
                    setProducts(mapped);
                    adminCache.set('admin_products', mapped);
                }

                const { data: realCategories } = await supabase.from('categories').select('*');
                if (realCategories) {
                    setCategories(realCategories);
                    adminCache.set('admin_categories', realCategories);
                }
        ...
        const unsubscribe = adminCache.subscribe((key) => {
            if (key === 'admin_products' || key === '*') {
                const fresh = adminCache.get('admin_products');
                if (fresh) setProducts(fresh);
            }
            if (key === 'admin_categories' || key === '*') {
                const freshCats = adminCache.get('admin_categories');
    ...
    const handleProductSubmit = async (e) => {
        ...
        if (selectedProduct) {
            ...
            const updatedList = products.map(p => p.id === selectedProduct.id ? { ...p, ...finalProduct } : p);
            setProducts(updatedList);
            adminCache.set('admin_products', updatedList);
            ...
            const updatedList = [newProdObj, ...products];
            setProducts(updatedList);
            adminCache.set('admin_products', updatedList);
    ...
    // Instant Filter & Search memoization
    const filteredProducts = useMemo(() => {
        ...
        return products.filter(p => {
            ...
            const matchCat = selectedCategoryFilter === 'all' || (p.category && p.category.toLowerCase() === selectedCategoryFilter.toLowerCase());

            const matchStock = stockFilter === 'all'
                ? true
                : stockFilter === 'in_stock' ? (p.stock > 0)
                : stockFilter === 'out_of_stock' ? (p.stock <= 0)
                : stockFilter === 'low_stock' ? (p.stock > 0 && p.stock < 10) : true;
    ...
    return (
        <>
            <div className="animate-fade-in space-y-8">
                ...
                <div className="flex flex-col md:flex-row gap-4 justify-between items-stretch md:items-center bg-stone-50 p-4 rounded-2xl border border-stone-200/70">
                    <div className="relative flex-1">
                        ...
                        <input
                            type="text"
                            placeholder="Instant search by title, SKU, or collection..."
                            value={searchQuery}
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <span className="text-[10px] font-bold tracking-widest uppercase text-stone-400 whitespace-nowrap pl-1">
                            {filteredProducts.length} / {products.length} pcs
                        </span>
                    </div>
                </div>
                            ...
                            {filteredProducts.length === 0 && (
                                <tr>
                                    <td colSpan="5" className="text-center py-12 text-stone-400 text-sm">
                                        {products.length === 0 ? "No products found. Create one!" : "No products match your search or filter."}
                                    </td>
                                </tr>
                            )}
                        </tbody>
            ...
            {isProductModalOpen && typeof document !== 'undefined' && createPortal(
                <div className="fixed inset-0 bg-stone-900/70 backdrop-blur-md z-[999999] flex items-center justify-center p-4 sm:p-6 overflow-y-auto min-h-screen" onPaste={handlePaste}>
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[90vh] my-auto animate-fade-in relative">
