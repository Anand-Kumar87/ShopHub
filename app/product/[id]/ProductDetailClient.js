...
export default function ProductDetailClient({ initialProduct, initialRelated }) {
    ...
    // Component States — seeded from SSR props (see page.js) so there's no loading flash
    ...
    // Scroll to top whenever a (new) product page is landed on
    useEffect(() => {
        window.scrollTo({ top: 0, behavior: 'instant' });
    }, [productId]);

    // 🔥 Data now arrives via SSR (see page.js) — no client-side fetch, no
    // loading spinner. This only covers the case where the server couldn't
    // find the product at all.
    useEffect(() => {
        if (!initialProduct) {
        ...
        <>
            ...
            <main className="bg-stone-50 min-h-screen pb-32 pt-24 animate-fade-in relative">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="bg-white rounded-[2.5rem] shadow-xl shadow-stone-200/40 w-full border border-stone-100 flex flex-col lg:flex-row overflow-hidden lg:rounded-none lg:shadow-none lg:border-0">
                                        ...
                                        <div className="mb-8">
                                            ...
                                            <div className="flex gap-3">
                                                {product.colors.map((color, idx) => (
                                                    ...
                                                    >
                                                        <span className="w-8 h-8 rounded-full block border border-stone-200/50" style={{ backgroundColor: resolveSwatchColor(color) }}></span>
                                                    </button>
                                                ))}
                                            </div>
                                        </div>
                                        ...
                                        <div className="border-b border-stone-200">
                                            ...
                                            {openAccordion === 'details' && (
                                                <div className="pb-5 text-sm text-stone-600 space-y-2 animate-fade-in">
                                                    ...
                                                    {product.colors?.length > 0 && <p><strong>Colors:</strong> {product.colors.map(c => resolveColorLabel(c)).join(', ')}</p>}
