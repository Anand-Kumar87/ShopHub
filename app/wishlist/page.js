...
export default function WishlistPage() {
    ...
    // 🔥 FIX: Simplified Move To Bag (Defaults to qty 1 since Modal is gone)
                        ...
                        {wishlistItems.map((item) => {
                            ...
                            return (
                                <div
                                    ...
                                    // 🔥 FIX: Directly push to the Master Product Page
                                ...
                                >
                                    <div className="relative aspect-[3/4] bg-stone-100 rounded-xl overflow-hidden mb-4 border border-stone-100/50">
                                        ...
                                        <button
                                            ...
                                            className="absolute top-3 right-3 w-8 h-8 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center text-stone-400 hover:text-red-500 hover:bg-white shadow-sm transition-colors z-10 lg:opacity-0 lg:group-hover:opacity-100"
                                    ...
                                    <div className="flex flex-col flex-grow px-1">
                                        ...
                                        <h3 className="text-sm font-bold text-stone-900 line-clamp-1 mb-1.5 group-hover:text-stone-600 transition-colors">{item.name}</h3>
                                        <p className="text-sm font-bold text-stone-900 mb-4">
                                            {convertPrice(item.salePrice || item.price)}
                                            {item.oldPrice && <span className="text-xs text-stone-400 line-through ml-2 font-normal">{convertPrice(item.oldPrice)}</span>}
                                        </p>
                                        ...
                                        <div className="mt-auto lg:hidden">
                                            {isAvailable ? (
                                                ...
                                                <button disabled className="w-full bg-stone-100 text-stone-400 text-[10px] font-bold tracking-widest uppercase py-3 rounded-full cursor-not-allowed">
                                                    Out of Stock
                                                </button>
                                            )}
                                        </div>
                                        ...
                                        {!isAvailable && (
                                            <div className="mt-auto hidden lg:block text-[10px] font-bold tracking-widest uppercase text-stone-400">Currently Unavailable</div>
                                        )}
                                    </div>
                                </div>
                            );
