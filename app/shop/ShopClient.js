...
function ShopContent({ initialCategories, initialProducts }) {
    ...
    useEffect(() => {
        if (swrCategories && swrCategories.length > 0) setAllCategories(swrCategories);
    }, [swrCategories]);
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={`relative bg-stone-100 rounded-xl overflow-hidden ${viewMode === 'list' ? 'w-1/3 aspect-square' : 'aspect-[3/4] mb-5'}`}>
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleWishlist(e, product); }}
                                                ...
                                                <button
                                                    ...
                                                    className={`absolute top-12 right-3 p-2 bg-white/90 backdrop-blur-sm rounded-full text-stone-400 hover:text-stone-900 hover:bg-white transition-all duration-300 z-10 opacity-100 sm:opacity-0 group-hover:opacity-100 translate-y-0 sm:translate-y-2 group-hover:translate-y-0 shadow-sm`}
                                                    title="Quick View"
                                                >
                                                    <FiEye size={16} />
                                                </button>
                                            ...
                                            <div className={viewMode === 'list' ? 'w-2/3 pr-4' : ''}>
                                                ...
                                                {viewMode === 'list' && (
                                                    <MagneticButton>
                                                        {isOutOfStock ? (
                                                            <button disabled className="px-8 py-3 bg-stone-200 text-stone-400 rounded-full text-sm font-bold tracking-widest uppercase cursor-not-allowed w-max" onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}>
                                                                Out of Stock
                                                            </button>
                                                        ) : (
                                                            <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleAddToCart(product); }} className="px-8 py-3 bg-stone-900 text-white rounded-full text-sm font-bold tracking-widest uppercase hover:bg-stone-800 transition-colors w-max">
                                                                Quick Add
                                                            </button>
                                                        )}
                                                    </MagneticButton>
