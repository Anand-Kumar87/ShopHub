...
export default function GlobalSettings() {
    ...
    // Load Settings on Mount with Admin Authorization
    useEffect(() => {
        const loadSettings = async () => {
            ...
            try {
                ...
                const dbRes = await fetch('/api/admin/settings', { headers, cache: 'no-store' });
                ...
                console.warn("API Error, loading local settings.", error);
                const localSettings = JSON.parse(localStorage.getItem('shophub_admin_settings'));
                if (localSettings) setSettingsForm(prev => ({ ...prev, ...localSettings }));
            } finally {
                setIsLoading(false);
    ...
    // Save Settings with Z+ Gateway Verification
    const handleSettingsSubmit = async (e) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errData.error || "Server rejected the save request.");
            }

            // Save local copy as backup
            localStorage.setItem('shophub_admin_settings', JSON.stringify(finalSettings));
            toast.success("Global preferences & credentials saved to Database!");
        } catch (err) {
            console.error("Save Error:", err);
            localStorage.setItem('shophub_admin_settings', JSON.stringify(finalSettings));
            toast.error(err.message || "Failed to save to database. Saved locally as fallback.");
        }
    };
