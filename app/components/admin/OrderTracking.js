...
import {
    FiX, FiCreditCard, FiSmartphone, FiBriefcase, FiSearch
} from 'react-icons/fi';
...
export default function OrderTracking() {
    ...
    useEffect(() => {
        ...
        const fetchOrders = async () => {
            ...
            try {
                ...
                if (data && !error) {
                    ...
                    adminCache.set('admin_orders', sortedOrders);
                }
                    ...
                    .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, payload => {
                        if (payload.eventType === 'INSERT') {
                            setOrders(prev => {
                                ...
                                adminCache.set('admin_orders', updated);
                                return updated;
                            });
        ...
        const unsubscribe = adminCache.subscribe((key) => {
            if (key === 'admin_orders' || key === '*') {
                const fresh = adminCache.get('admin_orders');
    ...
    // Instant Filtered Orders
    const filteredOrders = useMemo(() => {
        const query = searchQuery.trim().toLowerCase();
        return orders.filter(order => {
            const orderIdStr = (order.id || order.orderNumber || '').toString().toLowerCase();
            const clientEmail = (order.email || order.shipping?.email || order.shipping_address?.email || '').toLowerCase();
            const clientName = (order.customerName || (order.shipping ? `${order.shipping.firstName || ''} ${order.shipping.lastName || ''}` : '')).toLowerCase();

            const matchSearch = !query || orderIdStr.includes(query) || clientEmail.includes(query) || clientName.includes(query);
            const matchStatus = statusFilter === 'all' || (order.status || 'Processing').toLowerCase() === statusFilter.toLowerCase();
    ...
    return (
        <>
            <div className="animate-fade-in space-y-8">
                ...
                {/* Instant Search and Status Filter Pills */}
                <div className="flex flex-col md:flex-row gap-4 justify-between items-stretch md:items-center bg-stone-50 p-4 rounded-2xl border border-stone-200/70">
                    <div className="relative flex-1">
                        ...
                        <input
                            type="text"
                            placeholder="Instant search by Order Ref, client email, or customer name..."
                            value={searchQuery}
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold tracking-widest uppercase text-stone-400 whitespace-nowrap pl-2">
                            ...
                            {filteredOrders.length === 0 && (
                                <tr>
                                    <td colSpan="6" className="text-center py-12 text-stone-400 text-sm">
                                        {orders.length === 0 ? "Waiting for new orders..." : "No orders match your search or status filter."}
                                    </td>
                                </tr>
                            )}
                        </tbody>
            ...
            {/* ORDER PREVIEW MODAL VIA PORTAL */}
            {isOrderModalOpen && selectedOrder && typeof document !== 'undefined' && createPortal(
                <div className="fixed inset-0 bg-stone-900/70 backdrop-blur-md z-[999999] flex items-center justify-center p-4 sm:p-6 overflow-y-auto min-h-screen animate-fade-in">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh] my-auto relative">
