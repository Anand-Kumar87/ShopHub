...
import {
    ...
    FiSettings, FiGift, FiGlobe, FiCircle, FiMail, FiLock, FiRefreshCw
} from 'react-icons/fi';
...
export default function AdminDashboard() {
    ...
    // Z+ Security Gate: Verify session + profile role, and watch for revocation in real time
    useEffect(() => {
        let isMounted = true;
    ...
    const handleHardRefresh = () => {
        ...
        adminCache.clear();
        toast.success("Cache cleared! Live data refreshed.", { id: 'admin-refresh' });
        setTimeout(() => {
    ...
    if (!isAuthorized) {
        return (
            <div className="min-h-screen bg-stone-50 flex flex-col items-center justify-center animate-pulse">
                ...
                <p className="text-stone-500 font-bold tracking-widest uppercase text-xs">Verifying Z+ Security Credentials...</p>
            </div>
        );
    }
            ...
            <header className="bg-white border-b border-stone-200 px-6 py-5 flex justify-between items-center sticky top-0 z-30">
                ...
                <div className="flex items-center gap-4 text-stone-500">
                    <button
                        ...
                        className={`flex items-center gap-2 text-xs font-bold tracking-widest uppercase px-3 py-1.5 rounded-full border border-stone-200 hover:border-stone-900 hover:text-stone-900 transition ${isRefreshing ? 'opacity-50 pointer-events-none' : ''}`}
                    >
