...
import {
    FiPlus, FiEdit2, FiTrash2, FiX, FiCheck, FiClock, FiTag, FiActivity, FiSearch
} from 'react-icons/fi';
...
export default function CouponManagement() {
    ...
    const cachedCoupons = adminCache.get('admin_coupons');
    const [coupons, setCoupons] = useState(cachedCoupons || []);
    const [isLoading, setIsLoading] = useState(!cachedCoupons);
    ...
    const handleCouponSubmit = async (e) => {
        ...
        if (selectedCoupon) {
            ...
            const updated = coupons.map(c => c.id === selectedCoupon.id ? { ...c, ...finalData } : c);
            setCoupons(updated);
            adminCache.set('admin_coupons', updated);
    ...
    const filteredCoupons = coupons.filter(c => {
        ...
        if (!q) return true;
        return (c.code || '').toLowerCase().includes(q) || (c.target_role || '').toLowerCase().includes(q);
    });
    ...
    return (
        <>
            <div className="animate-fade-in space-y-8">
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-stone-200 pb-4">
                    <div>
                        ...
                        <p className="text-xs text-stone-400 mt-0.5">{coupons.length} Total Campaigns</p>
                    </div>
                                ...
                                <tr>
                                    <td colSpan="6" className="text-center py-16">
                                        ...
                                        <p className="text-stone-400 text-sm">
                                            {searchQuery ? 'No coupons match your search query.' : 'No promo campaigns are currently running.'}
                                        </p>
                                    </td>
                                </tr>
                            )}
            ...
            {/* 🔥 PREMIUM MODAL (PORTALED) */}
            {isMounted && isCouponModalOpen && createPortal(
                <div className="fixed inset-0 bg-stone-900/70 backdrop-blur-md z-[999999] flex items-center justify-center p-3 sm:p-6 overflow-y-auto animate-fade-in">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-stone-100 flex flex-col max-h-[90vh] my-auto">
                        ...
                        <form onSubmit={handleCouponSubmit} className="p-8 space-y-6 overflow-y-auto flex-1">
