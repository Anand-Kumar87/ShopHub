...
export default function HomeClient({ initialCategories, initialProducts }) {
    ...
    const { data: swrCategories } = useSWR('home_categories', fetchHomeCategories, {
        fallbackData: initialCategories?.length ? initialCategories : undefined,
        revalidateOnFocus: false,
        revalidateIfStale: false,
        revalidateOnReconnect: false
    });

    const { data: swrProducts } = useSWR('home_products', fetchHomeProducts, {
        fallbackData: initialProducts?.length ? initialProducts : undefined,
        revalidateOnFocus: false,
        revalidateIfStale: false,
        revalidateOnReconnect: false
    });
    ...
    useEffect(() => {
        if (swrCategories && swrCategories.length > 0) setCategories(swrCategories);
    }, [swrCategories]);

    useEffect(() => {
        if (swrProducts && swrProducts.length > 0) setAllProducts(swrProducts);
    }, [swrProducts]);
...
const shuffleArray = (arr) => {
    ...
    // Dynamic randomized product rotation for Home Page on each visit
    const [dynamicProducts, setDynamicProducts] = useState(() => {
        const active = (initialProducts || []).filter(p => p.status !== 'archived' && p.status !== 'draft');
        return active.slice(0, 8);
    });

    useEffect(() => {
        const active = (allProducts || []).filter(p => p.status !== 'archived' && p.status !== 'draft');
        if (active.length > 0) {
            // Shuffle so fresh and different products appear on each visit
            setDynamicProducts(shuffleArray(active).slice(0, 8));
        }
    }, [allProducts]);

    const products = dynamicProducts.length > 0 ? dynamicProducts : (allProducts || []).slice(0, 8);
    ...
    return (
        <main className="min-h-screen bg-stone-50 pb-20 animate-fade-in relative">
                ...
                <div className="relative h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-center">
                    <div className="max-w-xl">
                        ...
                        <div className="flex gap-4">
                            ...
                            <Link href="/categories" className="bg-white text-stone-900 border border-stone-300 px-8 py-3.5 rounded-full font-medium hover:bg-stone-50 transition-colors">
                                EXPLORE COLLECTIONS
                            </Link>
                        </div>
                    </div>
                        ...
                        products.map((product, index) => {
                            ...
                            return (
                                <Link
                                    ...
                                    className="group flex flex-col relative cursor-pointer block"
                                >
                                    <div className="relative aspect-[3/4] w-full bg-stone-100 rounded-xl overflow-hidden mb-4 block">
                                        ...
                                        <button
                                            type="button"
                                            onClick={(e) => { e.preventDefault(); e.stopPropagation(); setSelectedProduct(product); setModalQuantity(1); }}
                                            className="absolute top-14 right-3 p-2.5 bg-white/90 backdrop-blur-sm shadow-sm rounded-full text-stone-400 hover:text-stone-900 hover:bg-white transition-all z-10 opacity-100 sm:opacity-0 group-hover:opacity-100 translate-y-0 sm:translate-y-2 group-hover:translate-y-0 duration-300"
                                            title="Quick View"
                                        >
                                            <FiEye size={16} />
                                        </button>
                                        ...
                                        <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out z-10">
                                            {isOutOfStock ? (
                                                <button disabled className="w-full bg-stone-200 text-stone-400 font-bold py-3 rounded-lg cursor-not-allowed flex items-center justify-center gap-2" onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}>
                                                    Out of Stock
                                                </button>
                                            ) : (
                                                <button
                                                    type="button"
                                                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleAddToCart(e, product, 1); }}
            ...
            {/* FULL FEATURED ULTRA-PREMIUM QUICK VIEW MODAL */}
            {selectedProduct && (
                <QuickViewModal
