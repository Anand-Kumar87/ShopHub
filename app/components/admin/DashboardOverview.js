...
export default function DashboardOverview({ setActiveTab }) {
    ...
    useEffect(() => {
        ...
        const cachedCustomers = adminCache.get('admin_customers');

        if (cachedProducts) setProducts(cachedProducts);
        if (cachedOrders) setOrders(cachedOrders);
        if (cachedCustomers) {
            const emails = new Set();
            cachedCustomers.forEach(c => c.email && emails.add(c.email));
        ...
        const fetchDashboardData = async () => {
            if (!cachedProducts && !cachedOrders) setIsLoading(true);
            try {
                const { data: prodData } = await supabase.from('products').select('*');
                if (prodData) {
                    const sorted = prodData.sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));
                    const mapped = sorted.map(p => ({ ...p, images: p.images || (p.image_url ? [p.image_url] : []) }));
                    setProducts(mapped);
                ...
                if (ordData) {
                    const sorted = ordData.sort((a, b) => new Date(b.created_at || b.orderDate || 0) - new Date(a.created_at || a.orderDate || 0));
                    setOrders(sorted);
                    adminCache.set('admin_orders', sorted);
                }
        ...
        const unsubscribe = adminCache.subscribe((key) => {
            if (key === 'admin_products' || key === '*') {
                const p = adminCache.get('admin_products');
                if (p) setProducts(p);
            }
            if (key === 'admin_orders' || key === '*') {
                const o = adminCache.get('admin_orders');
                if (o) setOrders(o);
            }
        });
